...
class Board(Canvas):
	...
	def create_objects(self):
		...
		self.create_image(50, 50, image=self.head, anchor=NW, tag='head') # cabeça da cobra
		self.create_image(40, 50, image=self.body, anchor=NW, tag='body') # uma parte do corpo
		self.create_image(30, 50, image=self.body, anchor=NW, tag='body') # outra parte do corpo
	...
	def locate_dot(self):
		...
		r = randint(0, RAND_POS) #vai de 0 a 27
		self.dot_x = r * DOT_SIZE

		r = randint(0, RAND_POS)
	...
	def game_over(self):
		...
		self.delete(ALL)
		self.create_text(self.winfo_width()/2, self.winfo_height()/2, text='Game Over', fill='Red')
